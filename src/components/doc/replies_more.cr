class Comments::ListMore < BaseComponent
  ...
  private def render_emoji_buttons_and_delete_button(comment : Comment)
    ...
    footer class: "mt-2 flex flex-wrap items-end justify-between gap-x-4 gap-y-3" do
      ...
      render_thread_toggle(comment)
      render_comment_actions(comment, me) unless me.nil?
    end
  end

  private def render_thread_toggle(comment : Comment)
    return unless comment.parent_id.nil? && comment.descendants_count > 0
    ...
    div class: "shrink-0" do
      input type: "hidden", class: "comment-order-state", name: "order_by", value: pagination[:order_by]
