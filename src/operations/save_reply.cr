  ...
  before_save do
    ...
    if !id.value # 新建的时候
      ...
      parent_id.value.try do |parent_id|
        parent_reply = ReplyQuery.find(parent_id)
        #  - 如果父 reply 已经知道它属于哪个根 reply, 用父 reply 的 root_id
        #    即：至少是第三极评论，第一级 doc，第二级 root reply, 第三极才是父 reply

        # -  如果父 reply 没有 root_id, 说明父 reply 自己就是根评论, 因此就使用它的 id
        #    此时父 reply 就是上面的第二级 root reply
        thread_root_id = parent_reply.root_id || parent_reply.id
