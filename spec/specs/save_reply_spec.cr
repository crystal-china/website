...
describe SaveComment do
  it "requires exactly one comment target" do
    ...
    SaveComment.create(user_id: user.id, doc_id: doc.id, parent_id: parent.id, content: "invalid") do |operation, comment|
  ...
  it "assigns unique floors to concurrent comments" do
    ...
    10.times do |index|
      spawn do
        results.send SaveComment.create!(user_id: user.id, doc_id: doc.id, content: "comment #{index}")
      rescue error
        results.send error
      end
    end
